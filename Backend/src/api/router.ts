...
export function buildApiRouter(
  ...
  r.post("/trip/update", async (req: Request, res: Response) => {
    const { sessionId, patch } = req.body as {
      sessionId?: string;
      patch?: Partial<TripContext>;
    };
    if (!sessionId || !patch)
      return res.status(400).json({ error: "sessionId and patch required" });
  ...
  r.post("/poi/save", async (req: Request, res: Response) => {
    const { sessionId, poiId, saved } = req.body as {
      sessionId?: string;
      poiId?: string;
      saved?: boolean;
    };
    if (!sessionId || !poiId)
      return res.status(400).json({ error: "sessionId and poiId required" });
  ...
  r.post("/chat/send", async (req: AuthenticatedRequest, res: Response) => {
    const ip =
      ...
      req.ip ||
      "unknown";
    if (!opts.chatLimiter.allow(`chat:${ip}`))
      return res.status(429).json({ error: "Rate limit exceeded" });

    const {
      sessionId: incoming,
      inviteId: incomingInvite,
      message,
    } = req.body || {};
    ...
    try {
      ...
      if (session) {
        ...
        wsEvents = await opts.runRouter(
          {
            sessionId: sid,
            message,
            trip: (session?.trip as Partial<TripContext>) || {},
          },
          history,
        );
        ...
        for (const e of wsEvents) {
          ...
          if (e.type === "itinerary.update") {
            try {
              ...
              if (e.data && typeof e.data === "object" && e.data.daysPlan) {
                ...
                console.log(
                  `[router] Itinerary updated for session ${sid} with ${e.data.daysPlan?.length || 0} days`,
                );
              } else {
                console.warn(
                  `[router] Skipping invalid itinerary update for session ${sid}:`,
                  e.data,
                );
              }
            } catch (error) {
              console.error(
                `[router] Failed to persist itinerary for session ${sid}:`,
                error,
              );
            }
          }
          ...
          if (e.type === "search.results") {
            console.log(
              `[router] Persisting search results for session ${sid}:`,
              {
  ...
  r.get("/user/stats", async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      if (error) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to fetch user statistics" });
      }
      ...
      try {
        ...
        const rows = (sessions || []).map((s) => {
          ...
          const duration: string | null =
            typeof days === "number" ? `${days} days` : trip.duration || null;
          const title: string =
  ...
  r.get("/proxy/photo", async (req: Request, res: Response) => {
    ...
    if (!photo_reference || !key) {
      return res
        .status(400)
        .json({ error: "photo_reference and key required" });
    }
  ...
  r.get("/maps/api-key", async (req: Request, res: Response) => {
    ...
    if (apiKey.length !== 39 || !apiKey.startsWith("AIza")) {
      ...
      return res
        .status(500)
        .json({ error: "Invalid Google Maps API key format" });
    }
