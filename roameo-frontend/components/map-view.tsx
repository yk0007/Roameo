...
const CUSTOM_MAP_STYLE: any[] = [
  ...
  },
  {
    ...
    stylers: [{ saturation: 0 }, { visibility: "on" }, { color: "#fefefe" }],
  },
  {
    featureType: "road",
  ...
  },
  {
    featureType: "road",
    elementType: "labels.text.fill",
    stylers: [{ color: "#aca9a9" }, { visibility: "off" }],
  },
  {
    featureType: "road",
    elementType: "labels.text.stroke",
    stylers: [{ weight: 0.64 }, { color: "#393939" }, { visibility: "on" }],
  },
  {
  ...
  },
  {
    featureType: "road.highway",
    elementType: "labels.text.stroke",
    stylers: [{ weight: 2.99 }, { visibility: "on" }],
  },
  {
...
export default function MapView({
  ...
  const [addedOnly, setAddedOnly] = useState(
    !!(itineraryPoiIds && itineraryPoiIds.size > 0),
  );
  ...
  const [gmapsReady, setGmapsReady] = useState<boolean>(
  ...
  useEffect(() => {
    function onPoiHover(e: any) {
      ...
      infoWindowRef.current.setContent(
        `<div style="font-weight:600">${marker.getTitle?.() || "Place"}</div>`,
      );
        ...
        const showInfoWindow = (
          ...
          infoWindow.addListener("domready", () => {
            ...
            if (content) {
              ...
              const iwContainer = content.closest(
                ".gm-style-iw-c",
              ) as HTMLElement;
              const iwTitle = content
                .closest(".gm-style-iw")
                ?.querySelector(".gm-style-iw-t") as HTMLElement;
              const iwCloseBtn = content
                .closest(".gm-style-iw")
                ?.querySelector(".gm-ui-hover-effect") as HTMLElement;
              ...
              const iwContent = content.closest(
                ".gm-style-iw-d",
              ) as HTMLElement;
              if (iwContent) {
    ...
    <div className="relative h-full overflow-hidden">
      ...
      >
        ...
        {showFilterMenu && (
          <div
            ref={filterMenuRef}
            className="w-60 bg-white/95 backdrop-blur-md border border-white/30 rounded-2xl shadow-xl p-2 space-y-1"
          >
            <div
            ...
            >
              <div
                className={`w-4 h-4 rounded-full border ${filterAll ? "bg-black" : "bg-white"}`}
              />
            ...
            {(
              ...
              >
                <input
                  type="checkbox"
                  checked={filterAll ? true : filterTypes[t.key]}
                  readOnly
                  className="accent-black"
                />
                <span className="text-sm flex-1">
                  {t.label}{" "}
                  <span className="text-gray-400">{` ${pois.filter((p) => p.type === t.key).length}`}</span>
                </span>
              </div>
            ))}
            ...
            >
              ...
              <span className="text-sm flex-1">
                Saved only{" "}
                {savedIds ? (
                  <span className="text-gray-400">{` ${savedIds.size}`}</span>
                ) : null}
              </span>
            </div>
